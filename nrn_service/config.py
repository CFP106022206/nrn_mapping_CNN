...
class MatchConfig:
    ...
    # 通過三段過濾後再按 descriptor 距離取前 K。0 = 不截斷（預設）。
    #
    # 這個參數原本設 2000 當延遲護欄，但它不只是省時間 —— 它會改變輸出：候選被截掉時，
    # 第 K+1 名之後的會遞補進來打分，所以截斷版不是完整版的子集。實測 150 顆，
    # top_k=2000 時 EM->FC 只有 73.3% 的查詢給出和不截斷一樣的 top5（FC->EM 85.3%）。
    # 而且離線批次走的是同一條路徑（match_cli --batch_side），會一起被扭曲。
    #
    # 改成 0 的依據是全庫實測的延遲（不含模型載入）：
    #   EM->FC (12767 顆)  中位 352 ms  p99 1373 ms  最大 2014 ms（超過 2 秒的只有 1 顆）
    #   FC->EM (22286 顆)  中位 227 ms  p99  996 ms  最大 1620 ms
    # 網頁一次只查一顆，最壞 2 秒可接受；相較之下超大 EM 骨架光 render 就要 6.2 秒，
    # 打分階段不是尾巴的主角。順帶讓離線 CSV 與線上服務回傳一致。
    #
    # 參數保留：若之後併發變成瓶頸、或資料庫長大很多，設成非 0 可以換回延遲上限。
    # 參考候選數分佈（35°）：FC->EM 中位 1104 / p99 6057 / max 6880
    #                        EM->FC 中位 2001 / p99 8120 / max 9275
